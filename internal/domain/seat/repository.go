...
type Repository interface {
	...
	ReserveSeats(ctx context.Context, tx *sqlx.Tx, seatIDs []string, reservationID string) error

	// ConfirmSeats は座席を確定状態に更新する（トランザクション必須）
	ConfirmSeats(ctx context.Context, tx *sqlx.Tx, seatIDs []string) error

	// ReleaseSeats は座席を解放する（トランザクション必須）
	ReleaseSeats(ctx context.Context, tx *sqlx.Tx, seatIDs []string) error
