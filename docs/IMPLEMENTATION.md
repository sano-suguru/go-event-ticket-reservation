...
50人中1人だけが成功、残り49人は競合エラー—分散ロックが意図通りに動作しています。
...
| 指標 | 結果 | 評価 |
|------|------|------|
| スループット | 1,426 req/sec | 高スループット |
| p95 レスポンス | 40.23 ms | ✅ 閾値1000msをクリア |
| p99 レスポンス | 148.2 ms | ✅ 閾値2000msをクリア |
| エラー率 | 0.00% | ✅ |
...
3台にリクエストが分散されても、Redis の分散ロックで1人だけが予約に成功。

### 大規模データベンチマーク（10万座席）

**スタジアム規模（10万座席）での性能検証。**
